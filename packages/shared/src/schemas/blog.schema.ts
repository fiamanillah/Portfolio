...
export const blogStatusEnumSchema = z.enum(["DRAFT", "PUBLISHED", "SCHEDULED", "ARCHIVED"]);
export const blogArticleTypeEnumSchema = z.enum(["TechArticle", "BlogPosting", "Article"]);
...
export const listBlogPostsQuerySchema = z.object({
  ...
  featured: z
    ...
    .optional(),
  sortBy: z.enum(["publishedAt", "createdAt", "views", "likesCount", "title", "updatedAt"]).optional().default("createdAt"),
...
export const publicBlogQuerySchema = z.object({
  ...
  featured: z
    ...
    .transform((val) => (typeof val === "string" ? val === "true" : val))
    .optional(),
  sortBy: z.enum(["publishedAt", "views", "likesCount"]).optional().default("publishedAt"),
  sortOrder: z.enum(["asc", "desc"]).optional().default("desc"),
});

export const bulkBlogStatusSchema = z.object({
  ids: z.array(z.string().uuid()).min(1, "At least one blog post ID must be provided"),
