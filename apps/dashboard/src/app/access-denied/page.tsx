...
export default function AccessDeniedPage() {
  ...
  React.useEffect(() => {
    if (!isLoading && isAdmin) {
      toast.success("Admin Access Granted", {
        description: "Your administrator role has been verified. Redirecting...",
      });
      router.replace("/");
    }
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-background px-4 py-12">
      ...
      <div className="pointer-events-none absolute bottom-0 right-1/4 -z-10 h-80 w-80 rounded-full bg-destructive/10 blur-[100px]" />

      <div className="w-full max-w-md space-y-6">
        <div className="flex flex-col items-center text-center space-y-2">
          <div className="flex aspect-square size-14 items-center justify-center rounded-2xl bg-amber-500/10 text-amber-500 font-bold shadow-lg ring-8 ring-amber-500/10 border border-amber-500/20">
        ...
        <Card className="border-border/80 bg-card/80 backdrop-blur-xl shadow-2xl">
          <CardHeader className="pb-3 text-center">
            ...
            <CardDescription className="text-xs">
              You are currently authenticated, but your account lacks Super Admin clearance.
            </CardDescription>
          </CardHeader>

          <CardContent className="space-y-4">
            {/* User Profile Summary */}
            <div className="flex items-center gap-3 p-3.5 rounded-xl border border-border/80 bg-muted/30">
              ...
              <div className="grid flex-1 text-left text-xs leading-tight">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-sm truncate">{user?.name || "Authenticated User"}</span>
                  <Badge variant="secondary" className="font-mono text-[10px] uppercase">
                    {user?.role || "GUEST"}
                  </Badge>
                </div>
                <span className="text-muted-foreground truncate">{user?.email || "Unknown email"}</span>
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-xl border border-amber-500/20 bg-amber-500/5 text-xs text-muted-foreground space-y-1.5">
              ...
              <p className="leading-relaxed">
                This administrative console is reserved for Super Administrators to manage database records, email templates, subscribers, and system telemetry.
              </p>
            </div>
          </CardContent>

          <CardFooter className="flex flex-col gap-2 pt-2 pb-5 border-t border-border/50 pt-4">
            <Button
              variant="default"
            ...
            <Button variant="ghost" className="w-full gap-2 text-xs text-muted-foreground" asChild>
