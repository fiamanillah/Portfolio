...
import type { SkillListItemDTO } from "@workspace/shared"
      ...
      <AlertDialogContent className="w-[92vw] sm:w-auto sm:min-w-[500px] sm:max-w-none rounded-2xl border-border/80 bg-background/95 backdrop-blur-md p-6 shadow-2xl">
        <AlertDialogHeader>
          <AlertDialogTitle className="font-mono text-base font-bold uppercase">
            {isBulk
              ? `Delete ${bulkIds.length} skills?`
              : `Delete skill: ${skill?.name || ""}?`}
          </AlertDialogTitle>
          <AlertDialogDescription className="text-xs text-muted-foreground leading-relaxed">
            {isBulk
              ? `Are you sure you want to permanently remove ${bulkIds.length} skills from your portfolio? This action cannot be undone.`
              : `Are you sure you want to permanently remove "${skill?.name || ""}"? This action cannot be undone.`}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
