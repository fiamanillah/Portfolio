...
export default function CommentsModerationPage() {
  const [activeTab, setActiveTab] = React.useState<"comments" | "reports">("comments");
  ...
  const [reportReasonFilter, setReportReasonFilter] = React.useState<string>("ALL");
  ...
  const [activeReportToResolve, setActiveReportToResolve] = React.useState<CommentReportDTO | null>(null);
  const [reportAction, setReportAction] = React.useState<string>("DELETE_COMMENT");
  const [reportResolutionNotes, setReportResolutionNotes] = React.useState("");
  const [deleteConfirmTarget, setDeleteConfirmTarget] = React.useState<{ id: string; type: "single" | "bulk" } | null>(null);
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        if (res.pagination) {
          ...
          setCommentsTotalPages(res.pagination.pages || res.pagination.totalPages || 1);
        }
      }
    } catch (err) {
  ...
  const fetchReports = React.useCallback(async () => {
    ...
    try {
      const res = await CommentApi.getReports({
        ...
        status: reportStatusFilter !== "ALL" ? (reportStatusFilter as CommentReportStatus) : undefined,
        reason: reportReasonFilter !== "ALL" ? (reportReasonFilter as CommentReportReason) : undefined,
      });
      ...
      if (res.success && res.data) {
        ...
        if (res.pagination) {
          ...
          setReportsTotalPages(res.pagination.pages || res.pagination.totalPages || 1);
        }
      }
    } catch (err) {
  ...
  // Status update
  const handleUpdateStatus = async (id: string, status: CommentStatus, isPinned?: boolean) => {
  ...
  const handleBulkStatus = async (status: CommentStatus) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(`Updated ${res.data?.count || selectedCommentIds.length} comments to ${status}`);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(`Deleted ${res.data?.count || selectedCommentIds.length} comments`);
        setSelectedCommentIds([]);
        setDeleteConfirmTarget(null);
        fetchComments();
        fetchStats();
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            {stats.pendingReportsCount > 0 && (
              <Badge className="bg-rose-500 text-white font-mono text-[10px] animate-pulse">
                {stats.pendingReportsCount} FLAGGED ISSUES
              </Badge>
            )}
          </div>
          <p className="text-sm text-muted-foreground mt-1">
            Real-time management for community discussions, threaded replies, and user-flagged reports.
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as any)} className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b pb-3">
          <TabsList className="grid w-full grid-cols-2 sm:w-auto">
            ...
            <TabsTrigger value="reports" className="gap-2 text-xs relative">
              <ShieldAlert className="size-3.5 text-rose-500" />
              Reported Queue
              {stats.pendingReportsCount > 0 && (
                <span className="ml-1 rounded-full bg-rose-500 px-1.5 py-0.2 text-[10px] text-white font-bold">
        ...
        <TabsContent value="comments" className="space-y-4 mt-0">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between bg-card/60 p-3 rounded-lg border border-border/80">
            ...
            {selectedCommentIds.length > 0 && (
              <div className="flex items-center gap-2 bg-muted/60 px-3 py-1.5 rounded-md border border-primary/30 animate-in fade-in">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => setDeleteConfirmTarget({ id: "", type: "bulk" })}
                  disabled={actionLoading}
