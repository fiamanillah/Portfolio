...
export const CommentApi = {
   ...
   */
  async getStats() {
    return await request<CommentModerationStatsDTO>("/comments/v1/admin/stats", {
      method: "GET",
    });
  },
  ...
  async getComments(query: ListAdminCommentsQueryDTO = {}) {
    ...
    if (query.reportedOnly !== undefined) params.append("reportedOnly", String(query.reportedOnly));
    if (query.sortBy) params.append("sortBy", query.sortBy);
    if (query.sortOrder) params.append("sortOrder", query.sortOrder);

    const queryString = params.toString() ? `?${params.toString()}` : "";
    return await request<CommentAdminListItemDTO[]>(`/comments/v1/admin/comments${queryString}`, {
      method: "GET",
    });
  },
  ...
  async deleteComment(id: string) {
    return await request<{ success: boolean; id: string }>(`/comments/v1/admin/comments/${id}`, {
      method: "DELETE",
    });
  },
  ...
  async bulkUpdateStatus(payload: BulkCommentStatusDTO) {
    return await request<{ count: number }>("/comments/v1/admin/comments/bulk-status", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  },
  ...
  async bulkDelete(payload: BulkCommentDeleteDTO) {
    return await request<{ count: number }>("/comments/v1/admin/comments/bulk-delete", {
      method: "POST",
      body: JSON.stringify(payload),
    });
  },
  ...
  async getReports(query: ListAdminReportsQueryDTO = {}) {
    ...
    const queryString = params.toString() ? `?${params.toString()}` : "";
    return await request<CommentReportDTO[]>(`/comments/v1/admin/reports${queryString}`, {
      method: "GET",
    });
  },
  ...
  async deleteReport(id: string) {
    return await request<{ success: boolean; id: string }>(`/comments/v1/admin/reports/${id}`, {
      method: "DELETE",
    });
  },
};
