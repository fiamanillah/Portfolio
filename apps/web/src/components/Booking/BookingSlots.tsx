        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 md:grid-cols-4">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div key={i} className="h-11 animate-pulse border border-border/50 bg-muted/20" />
            ))}
          </div>
        ) : slots.length === 0 ? (
          // Empty State
          <div className="relative border border-dashed border-border/80 bg-background/50 p-8 text-center">
            ...
            <p className="font-mono text-xs font-bold text-foreground">No available time slots on this date</p>
            <p className="mt-1 font-mono text-[11px] text-muted-foreground">
              All slots for this day are either booked or outside working availability.
            </p>
            <button
              type="button"
              onClick={onBackToDate}
