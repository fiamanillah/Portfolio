...
import { TIMEZONE_REGIONS } from "./types"
import {
  Select,
  SelectTrigger,
  SelectValue,
...
export function BookingDateTimeStep({
  ...
  return (
    <div className="flex flex-1 flex-col justify-between">
      {/* ── Main Side-by-Side Panel (Calendar + Available Times) ─────── */}
                  ...
                  className={[
                    ...
                    isSelected
                      ...
                      : isDisabled
                        ...
                        : !item.isCurrentMonth
                          ? "cursor-pointer text-muted-foreground/40 hover:bg-muted/40 hover:text-foreground"
          ...
          <div className="max-h-[260px] flex-1 space-y-2 overflow-y-auto pr-1">
            {isLoadingSlots ? (
              ...
              Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="h-9 w-full animate-pulse rounded-lg border border-border/40 bg-muted/20"
                />
              ))
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      isSelected
                        ? "border-primary bg-primary font-bold text-primary-foreground shadow-sm ring-1 shadow-primary/25 ring-primary"
                        : slot.available
                          ? "cursor-pointer border-border/80 bg-background text-foreground hover:border-primary/60 hover:bg-primary/5 hover:text-primary"
      ...
      {/* ── Timezone Selector (Directly attached below, no gap) ─────── */}
      <div className="border-t border-border bg-muted/5 px-4 py-3 sm:px-5">
      ...
      {selectedSlot && onProceedToDetails && (
        <div className="border-t border-border bg-background/50 p-4 sm:px-5">
          <button
            type="button"
            onClick={onProceedToDetails}
            className="btn-shimmer flex w-full items-center justify-center gap-2 rounded-lg border border-primary bg-primary py-3 font-mono text-xs font-bold tracking-wider text-primary-foreground uppercase transition-all hover:opacity-90 active:scale-[0.99]"
          >
