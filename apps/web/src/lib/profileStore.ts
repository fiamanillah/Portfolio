...
export async function updateProfileInfo(updates: UpdateProfileInput): Promise<{
  ...
  try {
    ...
    return { success: false, error: res.error || res.message || "Failed to update profile." };
  } catch (err: any) {
...
export async function updateSubscriptionStatus(subscribed: boolean): Promise<{
  ...
  try {
    ...
    return { success: false, error: res.error || res.message || "Failed to update subscription." };
  } catch (err: any) {
