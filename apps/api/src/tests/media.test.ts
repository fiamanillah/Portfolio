...
describe("Cloudflare R2 / S3 Storage Engine & Media Management Integration Tests", () => {
  ...
  const realS3Client = new S3Client({
    region: "auto",
    endpoint:
      "https://60656a81bb1a0701ad39ec1287c88b0a.r2.cloudflarestorage.com",
    credentials: {
  ...
  beforeAll(async () => {
    ...
    const passwordHash = await Bun.password.hash("SecurePassword123!", {
      algorithm: "bcrypt",
      cost: 10,
    })
  ...
  it("1. StorageService should format collision-safe S3 object keys and public URLs", () => {
    const key = storageService.generateObjectKey(
      "blogs",
      "Hero Banner (Final) - 2026.PNG"
    )
    expect(key).toStartWith("blogs/")
    expect(key).toEndWith(".png")
    expect(key).toContain("hero-banner-final-2026")

    const publicUrl = storageService.formatPublicUrl(
      "blogs/2026/08/banner-123.webp"
    )
    expect(publicUrl).toBe(
      "https://assets.fi.amanillah.com/blogs/2026/08/banner-123.webp"
    )
  ...
  it("9. MediaService should aggregate accurate storage stats", async () => {
    ...
    expect(stats.folders.some((f) => f.folder === "test-storage-blogs")).toBe(
      true
    )
  })
  ...
  it("11. MediaService should allow admin or owner to delete media file", async () => {
    const deleteRes = await mediaService.deleteMedia(
      testMediaId,
      testUser.id,
      Role.ADMIN
    )
  ...
  it("13. MediaService should deduplicate identical uploads to save storage and API costs", async () => {
    const identicalPayload = Buffer.from(
      "identical file payload for deduplication test"
    )
    const mockFile: Express.Multer.File = {
      fieldname: "file",
  ...
  it("14. MediaService should scan and cleanup orphaned and unreferenced media files", async () => {
    ...
    expect(cleanupRes.keys).toContain(
      "avatars/2026/08/orphaned-avatar-test.jpg"
    )
  })
})
