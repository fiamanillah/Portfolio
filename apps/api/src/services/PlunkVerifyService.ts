...
export class PlunkVerifyService {
   ...
   */
  public static async verifyEmail(email: string): Promise<PlunkVerifyResult | null> {
    const cleanEmail = email.trim().toLowerCase();
    const secretKey = config.plunk.secretKey;

    if (this.isPlaceholderKey(secretKey)) {
      this.logger.warn("⚠️ PLUNK_SECRET_KEY missing or placeholder. Skipping Plunk POST /v1/verify in dev mode.");
      return null;
    }
    ...
    try {
      ...
      if (!resData || !resData.success || !resData.data) {
        this.logger.warn(`Plunk verify returned unexpected structure for ${cleanEmail}`, { resData });
        return null;
      }
      ...
      if (data.valid === false || data.hasMxRecords === false || data.domainExists === false || data.isTypo) {
        const domain = cleanEmail.split("@")[1] || cleanEmail;

        if (data.isTypo && data.suggestedEmail) {
          this.logger.warn(`Typo detected in email address: ${cleanEmail} -> Suggested: ${data.suggestedEmail}`);
          throw new BadRequestError(
        ...
        const reasonMsg = data.reasons && data.reasons.length > 0
          ? data.reasons.join(". ")
          : "The email address provided is invalid and cannot receive messages.";

        this.logger.warn(`Email verification failed for ${cleanEmail}: ${reasonMsg}`);
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        if (resData?.data?.isDisposable || resData?.message?.includes("disposable")) {
          throw new BadRequestError(
            "Disposable or temporary email addresses are strictly prohibited. Please use your primary email address.",
            { isDisposable: true }
          );
      ...
      this.logger.warn("Plunk verify check encountered network/service issue, allowing formatted email", { error });
      return null;
    }
  }
}
