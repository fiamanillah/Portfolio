...
export class StorageService {
  ...
  constructor(s3Client?: S3Client, bucketName?: string, publicDomainUrl?: string) {
    this.bucket = bucketName || config.storage.bucket;
    this.publicDomain = (publicDomainUrl || config.storage.publicDomain || "").replace(/\/$/, "");
   ...
   */
  public generateObjectKey(folder: string = "general", fileName: string): string {
  ...
  public validateMimeType(mimeType: string): void {
    if (config.storage.allowedMimeTypes && config.storage.allowedMimeTypes.length > 0) {
   ...
   */
  private formatTagsMetadata(tags?: string[] | Record<string, string>): string | undefined {
   ...
   */
  public async uploadBuffer(options: UploadBufferOptions): Promise<UploadResult> {
    const {
      buffer,
      fileName,
      mimeType,
    ...
    const command = new PutObjectCommand({
      ...
      ContentDisposition: contentDisposition || `inline; filename="${path.basename(fileName)}"`,
      Metadata: {
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          this.logger.info(`✔ Auto-created R2/S3 bucket "${this.bucket}". Retrying upload...`);
          response = await this.s3.send(command);
        } catch (createErr: any) {
          this.logger.warn(
          ...
          if (!config.server.isProduction) {
            ...
            this.logger.info(`📁 [Dev Fallback] Saved asset to local upload storage: ${localFilePath}`);
  ...
  public async getObjectStream(key: string): Promise<{
    ...
    if (!res.Body) {
      throw new NotFoundError(`Object stream for "${key}" is empty or not found`);
    }
   ...
   */
  public async deleteObjects(keys: string[]): Promise<{ deleted: string[]; errors: string[] }> {
