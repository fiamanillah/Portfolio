...
export class ContactController extends BaseController {
  ...
  public async submitContactForm(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!isCaptchaValid) {
        throw new BadRequestError("Security check failed. Please refresh and try submitting again.");
      }

      // ── Stage 4a: Input Sanitization ──────────────────────────────
      const sanitizedName = this.contactService.sanitizeInput(payload.name);
      const sanitizedSubject = this.contactService.sanitizeInput(payload.subject || "Website Inquiry");
      const sanitizedMessage = this.contactService.sanitizeInput(payload.message);
