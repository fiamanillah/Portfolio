...
export class AuthServices {
  ...
  public async initiateRegistration(dto: InitiateRegisterDTO) {
    ...
    if (hp_field && hp_field.trim().length > 0) {
      this.logger.warn("⚡ Bot trap triggered during registration attempt", {
        email,
      })
      return {
        success: true,
    ...
    if (existingUser && existingUser.isEmailVerified) {
      this.logger.warn(
        "Registration rejected: Email already registered and active",
        { email }
      )
    ...
    try {
      ...
      this.logger.error("Failed to send registration OTP email", {
        sendError,
        email,
      })
    }
  ...
  public async verifyRegisterOtp(
    ...
    if (!record.payload) {
      throw new BadRequestError(
        "Corrupted registration payload. Please start registration again."
      )
    }
    ...
    if (payload.subscribedToNewsletter) {
      try {
        ...
        this.logger.warn("Failed to auto-subscribe user to newsletter", {
          subErr,
          email,
        })
      }
    ...
    this.logger.info("✔ User registration and activation completed", {
      userId: user.id,
      email,
    })
  ...
  public async forgotPassword(dto: ForgotPasswordDTO) {
    ...
    if (!user) {
      this.logger.warn("Password reset requested for non-existing email", {
        email,
      })
      return {
        success: true,
        message:
          "If an account is associated with this email, a verification code has been dispatched.",
      }
    }
  ...
  public async verifyResetOtp(dto: VerifyResetOtpDTO) {
    ...
    return {
      ...
      email,
      message:
        "Verification code verified successfully. You may now set a new password.",
    }
  }
