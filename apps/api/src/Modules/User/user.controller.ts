...
export class UserController extends BaseController {
  ...
  public async uploadAvatar(req: Request, res: Response) {
    ...
    if (!req.file) {
      return res.status(400).json({
        success: false,
        message:
          "No image file provided. Please upload an image file (PNG, JPG, WebP, GIF, SVG).",
      })
    }
  ...
  public async listUsersAdmin(req: Request, res: Response) {
    ...
    const { data, pagination, stats } =
      await this.userService.listUsersAdmin(query)
  ...
  public async updateUserRoleAdmin(req: Request, res: Response) {
    ...
    const updated = await this.userService.updateUserRoleAdmin(
      currentAdminId,
      targetUserId,
      dto
    )
  ...
  public async deleteUserAdmin(req: Request, res: Response) {
    ...
    const result = await this.userService.deleteUserAdmin(
      currentAdminId,
      targetUserId
    )
    return this.sendResponse(req, res, result.message, 200, result)
