...
export class R2StorageProvider implements InfrastructureProvider<S3Client> {
  ...
  constructor(options?: StorageProviderOptions) {
    ...
    const secretAccessKey = options?.secretAccessKey || config.storage.secretAccessKey;
    ...
    this.client = new S3Client({
      ...
      credentials:
        accessKeyId && secretAccessKey
          ...
          : undefined,
      forcePathStyle: options?.forcePathStyle ?? (endpoint?.includes("localhost") || endpoint?.includes("127.0.0.1")),
    });
  ...
  public async connect(): Promise<void> {
    ...
    if (config.storage.accessKeyId && config.storage.secretAccessKey) {
      try {
        ...
        this.logger.info(`✔ Connected and verified R2/S3 Bucket: "${this.bucket}"`);
      } catch (err: any) {
        if (
          err.name === "NotFound" ||
          ...
          try {
            ...
            this.logger.info(`✔ Successfully auto-created Cloudflare R2 bucket: "${this.bucket}"`);
          } catch (createErr: any) {
            this.logger.warn(
      ...
      this.logger.info("ℹ️ R2/S3 Storage initialized without active cloud credentials (dev/mock mode)");
    }
  }
  ...
  private mapStorageError(err: unknown): AppError | null {
    if (err instanceof S3ServiceException) {
      ...
      const statusCode = err.$metadata?.httpStatusCode || HTTPStatusCode.BAD_GATEWAY;

      switch (errorName) {
        case "NoSuchKey":
        case "NotFound":
          return new NotFoundError("Requested object was not found in storage", {
        ...
        case "SignatureDoesNotMatch":
          return new AuthorizationError("Storage authorization failed. Please check your Cloudflare R2 credentials.", {
            s3Code: errorName,
          });

        case "EntityTooLarge":
        case "MaxMessageSizeExceeded":
          return new PayloadTooLargeError("File size exceeds maximum storage limit");

        case "SlowDown":
        case "TooManyRequests":
          return new RateLimitError("Object storage rate limit exceeded. Please retry momentarily.");
